...
class ScoreTableTournamentClubPipeline:
    ...
    def get_competitions_with_seasons(self) -> List[Dict[str, Any]]:
        ...
        try:
            with self.db_manager:
                ...
                logger.info(f"Found {len(club_competitions)} club tournament competitions in database")
    ...
    def scrape_score_tables_for_competition(self, competition: Dict[str, Any]) -> bool:
        """
        Scrape score tables for all seasons of a single CLUB TOURNAMENT competition.
        ...
        try:
            ...
            with self.db_manager:
                self.db_manager.insert_tournament_score_tables(
                    'score_table_club_tournament', competition_name, competition_id, score_tables_by_season
                )
    ...
    def scrape_score_tables(self, competition_id: Optional[int] = None):
        """
        Run the complete score table scraping pipeline for club tournaments.
        
        Args:
            competition_id: Specific competition ID to scrape, or None for all
        """
        logger.info("Starting Club Tournament Score Table Scraping Pipeline")
        ...
        try:
            ...
            if not competitions:
                logger.warning("No club tournament competitions with seasons found to scrape score tables for")
                return
            ...
            # Simple summary
            logger.info(f"✅ Club Tournament Score Table Scraping Completed: {successful_scrapes} successful, {failed_scrapes} failed")
